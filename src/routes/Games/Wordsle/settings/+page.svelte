<script lang="ts">
    ...
    onMount(() => {
        const storedOptions = localStorage.getItem("wordsle-options");
        if (storedOptions) {
...
<svelte:head>
    <title>Wordsle Settings</title>
    <meta name="description" content="Configure the settings of Wordsle (doing so may subtract from your score)" />
...
<div class="page">
    <main>
        <form on:submit|preventDefault={() => {
            if (!browser) return;
            localStorage.setItem("wordsle-options", JSON.stringify({
                ...options,

<script lang="ts">
    ...
    async function calculateScore(word?: string) {
        ...
        if (score > highscore) {
            highscore = score;
            localStorage.setItem('wordsle-highscore', highscore.toString());
        }
    }
    ...
    onMount(async() => {
        ...
        options = {
            ...options,
            ...JSON.parse(localStorage.getItem('wordsle-options') ?? '{}')
        }

        highscore = parseInt(localStorage.getItem('wordsle-highscore') ?? '0');
...
<svelte:head>
    <title>Wordsle</title>
    <meta name="description" content="Wordle but the answer is always 'words'">
    <meta name="keywords" content="wordle, words, wordsle">
...
<div class="page">
    ...
    <a class="settings-link" href="Wordsle/settings">Settings</a>
</div>

<script lang="ts">
    ...
    onMount(() => {
        const storedOptions = localStorage.getItem("jobhavle-options");
        if (storedOptions) {
...
<svelte:head>
    <title>Jobhavle Settings</title>
    <meta name="description" content="Configure the settings of Jobhavle (doing so may subtract from your score)" />
...
<div class="page">
    <main>
        <form on:submit|preventDefault={() => {
            if (!browser) return;
            localStorage.setItem("jobhavle-options", JSON.stringify({
                ...options,

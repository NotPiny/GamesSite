<script lang="ts">
    ...
    async function calculateScore(word?: string) {
        ...
        if (score > highscore) {
            highscore = score;
            localStorage.setItem('jobhavle-highscore', highscore.toString());
        }
    }
    ...
    onMount(async() => {
        ...
        options = {
            ...options,
            ...JSON.parse(localStorage.getItem('jobhavle-options') ?? '{}')
        }

        highscore = parseInt(localStorage.getItem('jobhavle-highscore') ?? '0');

        gameState.update((state) => {
            inputHint = `Enter a word${options.showLength ? ` (${fixedWord.length} letters)` : ''}`;
    ...
    async function processInput() {
        ...
        if (!words.includes(input.value.toLowerCase()) && !options.allowInvalid) return flashHint('Invalid word (only valid word is jobhaver)');
...
<svelte:head>
    <title>Jobhavle</title>
    <meta name="description" content="Wordle but the answer is always 'jobhaver'">
    <meta name="keywords" content="wordle, jobhaver, jobhavle">
...
<div class="page">
    ...
    <a class="settings-link" href="Jobhavle/settings">Settings</a>
</div>
